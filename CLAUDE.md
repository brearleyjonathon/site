...
- The `.shard`, `.diver`, `.splash` and `.drop` rules sit after the warp, deliberately. They used to sit between the blob rules and the grain, where two separate rewrites of the field section swept them away without any error: the words still got wrapped and the figure still got built, they simply had no `position` or animation left, so nothing moved. Keep them below the field, and after changing that section check that a word actually shifts rather than that the spans exist. A click drops a stick figure that arcs into the page and splashes. Every word in `main` is also wrapped in a `.shard` span while Fun is on and unwrapped when it is off; words near the pointer are nudged aside with `left` / `top` and eased back by a CSS transition.
